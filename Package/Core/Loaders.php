...
class Loaders{
	...
	public function _initialize($app){
		...
		$this->helper('/file');	//加载文件操作函数
		$this->helper('/versions');	//php内置函数版本兼容库
	 ...
	 * 	加载类库
	 */
	public function library($className,$key = false){
		if(!$this->_get_instance) $this->_get_instance = get_instance();
		...
		return $this->_get_instance->$instanceName = $Object;
