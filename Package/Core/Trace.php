...
class Trace{
	...
	static public $runtime_end = 0;

	static public $memory_start = 0;
	static public $memory_end = 0;




	static public $trace_info = array(
		'load_file' =>	array(),
		'load_config' =>	array(),
		'sql' =>	array(),
		'debug' =>	array(),
	...
	static function show(){
		...
		$basic['request'] = date('Y-m-d H:i:s',$_SERVER['REQUEST_TIME']).' '.$_SERVER['SERVER_PROTOCOL'].' '.$_SERVER['GATEWAY_INTERFACE'];
		$basic['request'] = date('Y-m-d H:i:s',$_SERVER['REQUEST_TIME']).' '.$_SERVER['SERVER_PROTOCOL'].' '.$_SERVER['GATEWAY_INTERFACE'];
		...
		foreach(self::$trace_info as $key => $value){
			...
			$count = count($value);
			$basic['count_file_load'] += $count;
			$menu .= '<span style="color:#000;padding-right:12px;height:30px;line-height: 30px;display:inline-block;margin-right:3px;cursor: pointer;font-weight:700">'.self::$keys[$key].' ('.$count.')</span>';

			$html .= '<div style="display:none;"><ol style="padding: 0; margin:0">';
			foreach($value as $k=>$v){
				$html .= '<li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">';
				$html .= $v;
